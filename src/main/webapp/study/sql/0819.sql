...
-- ALTER TABLE student ADD FOREIGN KEY student_idfk_1;
...
-- springgroup데이터베이스의 student테이블의foreing key삭제??
...
-- left join = left outer join :왼쪽테이블이 기준이고 오른쪽테이블이 조합한다(즉, 왼쪽테일블의 모든 필드가 출력된다)
...
-- Light join = Light outer join :오른쪽테이블이 기준이고 왼쪽테이블이 조합한다(즉, 오른쪽 테이블의 모든 필드가 출력된다)
SELECT * FROM hakkwa h LEFT JOIN student s ON h.code = s.hakkwa_code ;
SELECT * FROM hakkwa h LEFT JOIN student s ON h.code = s.hakkwa_code ORDER BY h.name;
...
-- 합집합: 완전 외부조인 (full outer join) -mysql 지원하징 않는다
-- SELECT * FROM hakkwa h FULL OUTER JOIN student s;
-- MySQL 에서는 union을 이용하려 합집합으 개면으로 사용한다 , Merge(병합)
-- 해당 테이블의 필드명이 같으면 병합테이블로 만들어주지만 필드명이 다르면 왼쪽테이블 기준으로 처리한다.
