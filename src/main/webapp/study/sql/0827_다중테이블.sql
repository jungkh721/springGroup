...
-- 1번 이강 구매한 사람들의 정보를 구매테이블에서 확인
select *from orders GROUP BY custid;

-- 1번 이상 구매한 책들의 정보?(책 이름과 함께 출력)
...
-- 책을 구입한 사람들의 이름/책이름/구매가격/구매날자??(단 , 구매자가 구매한 책의 권수와,구매총금액, 구매평균과 함께 출력)	
-- 단 구매한 책이4권 이상인 자료만 출력(구매일자 내림차순 출력)
select c.customer, count(o.custid),format(sum(o.saleprice),0),format(avg(o.saleprice),0), b.bookname, o.saleprice, o.orderdate
	from orders o, books b, customers c
	where o.bookid = b.bookid and o.custid = c.custid
...
--2025년에 최고로 많이 팔린 책을 이름과 출판사명과 권수를 추력하지~~	
select b.bookname, b.publisher
	...
	/*----------------------------------------------------------------*/
	-- 서브쿼리 (sub Query):하나의 SQL문장안에서 또다른 또다른 SQLansdmf vhgkatlzlsmsrjt
...
select o.custid, c.customer from otderid o, customers c where o.custid =c.custid;	
select o.custid, c.customer from otderid o, customers c where o.custid =c.custid group by ;	
select custid customer from customers where custid in (select custid from orders);

-- 구매한 도거중에서 가장 비싼 도서명은 어떤것인지, '도서명'과 '구매자'를 출력
select (select bookname from books where bookid=orders.bookid) as bookname 
	from orders order by saleprice limit 1;

-- 대한미디어 ,삼성당 에서 출판한 도서를 구매한 고객의 이름??(서브쿼리 사용~~~)

select customer from customers	
  where custid in(( select custid from orders o 
  where o.bookid in (select bookid from book where publisher='대한미디어' or publisher='삼성당'));
  

--숙제????ㅜㅜ  
-- 1.'손흥민'이 구매한 도서의 이름, 정가 구매가격, 할인받은 금액(정가와 판매가격의 차)를 출력하시오
-- 2.고객별로 주문한 도서의 총수량과 총판매액을 구하시오, 이때 고객아이디도 함께 출력하시오
-- 3.도서의 가격과 판매가격의 차가 가장 많이 나는 주문을 출력하시오
-- 4.고객별 평균 구매액을 구하시오,단 출력은 고객명, 평균구매액을 출력하시오.  
-- 5.'2025년5월1일' 이후부터 최근가지 판매된 책의 이름과 판매금액을 출력하시오	
